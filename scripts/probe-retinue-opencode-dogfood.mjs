...
const TASKS = [
  ...
  },
  {
    ...
    message:
      "Read-only bounded review. Use at most 4 repository inspection tool calls. Inspect src/backends/opencode/backend.ts, src/mcp.ts, plugins/retinue/retinue.config.json, and docs/reference/backends/opencode.md only if needed. Treat retinue.config.json as packaged defaults, not persistent user configuration. Do not propose patches, do not output unified diffs, and do not write files. Task: judge whether the default OpenCode explore path delegates read-only behavior to the OpenCode-native agent/profile and session permission boundary while leaving Retinue prompt contracts and prompt-level tools:false maps opt-in rather than mandatory. Do not require src/mcp.ts to pass readOnly:true; current product semantics intentionally avoid Retinue-owned read-only overlays. Final answer must include PASS or FAIL, 1-3 evidence lines with file references, and end with RETINUE_DOGFOOD_READONLY_DONE."
  },
  {
