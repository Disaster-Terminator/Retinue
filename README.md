...
- 同一 `RETINUE_STATE_DIR` 下还会执行机器级 active-agent 预算，避免多个 Codex/Hermes MCP server 把本地槽位相乘。`RETINUE_MAX_CONCURRENT_AGENTS` 只限制单个 MCP server 会话，包默认值为 `3`；`RETINUE_GLOBAL_AGENT_BUDGET` 限制共享 state dir 下的总 active 子代理数，未设置时默认是 `max(5, RETINUE_MAX_CONCURRENT_AGENTS)`。队列长度由 `RETINUE_MAX_QUEUED_AGENTS` 限制，默认 `20`；队列满时新的 spawn 返回 `resource_exhausted` 和 `reason: "queue_full"`，不会继续创建后端子代理。
