...
官方脚本默认把 OpenCode 安装到 `$HOME/.opencode/bin/opencode`。Retinue 也兼容常见 npm/pnpm/bun 全局安装路径，但 0.2.0 的默认文档和冒烟验收以官方脚本安装为准。
...
- `explore` 是 0.2.0 的默认 agent。Retinue 不再提供产品级 `access_mode`，也不再用自己的 read-only prompt/tool 覆盖 OpenCode 行为。
...
- 同一 `RETINUE_STATE_DIR` 下还会执行机器级 active-agent 预算，避免多个 Codex/Hermes MCP server 把本地槽位相乘。`RETINUE_MAX_CONCURRENT_AGENTS` 只限制单个 MCP server 会话；`RETINUE_GLOBAL_AGENT_BUDGET` 限制共享 state dir 下的总 active 子代理数，未设置时默认沿用会话上限。超预算时新的 spawn 返回 `resource_exhausted`，不会继续创建后端子代理。
...
- [0.2.0 发布说明（中文）](docs/release/v0.2.0_RELEASE_NOTES.zh-CN.md)
- [0.2.0 发布就绪记录](docs/release/0.2.0_RELEASE_PLAN.md)
