...
Retinue selects the backend from deployment configuration. The default Hermes integration uses OpenCode with the `plan` agent, Retinue-managed OpenCode server lifecycle, and a read-only OpenCode session policy. Read-only mode denies edits and nested `task` agents, denies `doom_loop` and interactive `question` prompts for headless runs, and sends a prompt-level override that disables OpenCode `bash`, `edit`, `write`, `apply_patch`, and `task` tools even when the active OpenCode profile allows them. Retinue also prepends a short read-only capability contract to the child prompt so the child knows to use `read`, `grep`, and `glob`, avoid shell/write attempts, avoid unified diffs or patch blocks in review output, and report the limitation when the task needs unavailable capabilities. This keeps default review work from inheriting shell access, writing files, or recursively delegating to nested OpenCode task agents while preserving normal repository inspection through OpenCode's `read`, `grep`, and `glob` tools. It also means default read-only children cannot inspect git history, staged changes, uncommitted diffs, or the latest commit unless the caller provides the relevant file paths or content; use `profile` access only when git commands are intentionally acceptable. Set `RETINUE_OPENCODE_ACCESS_MODE=profile` or the older `RETINUE_OPENCODE_READ_ONLY=0` only when child-agent shell access, writes, and nested delegation are intentionally acceptable. A single spawn may pass `access_mode: "profile"` for the same purpose. Each Retinue MCP server session keeps up to 3 active child agents by default. A spawn beyond the limit closes the oldest active child and returns `evictedJobId`; deployments can tune this with `RETINUE_MAX_CONCURRENT_AGENTS`. Do not pass backend, model, provider, profile, or OpenCode server choices in tool arguments.
...
5. If wait returns `running`, first inspect the returned `diagnostic`, `stdoutTail`, and `stderrTail`; then use `stateDir`, `tracePath`, `jobDir`, `stdoutPath`, and `stderrPath` when deeper diagnosis is needed. Complex OpenCode `plan` jobs can spend several minutes in tool-call rounds before producing final text, so poll again unless the task reaches a terminal state. If wait returns `stalled`, inspect `diagnostic.stallReason` and `diagnostic.stallSummary` when present, treat the child as non-evidence, and close it after preserving the returned paths for diagnosis.
