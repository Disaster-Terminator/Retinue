...
Retinue selects the backend from deployment configuration. The default Hermes integration uses OpenCode with the built-in `explore` subagent and Retinue-managed OpenCode server lifecycle. OpenCode keeps ownership of profile, provider, model, tools, and agent permissions; Retinue does not expose a product-level `access_mode` or overlay its own read-only prompt/tool policy on normal OpenCode children. A single spawn may pass `agent` to select an OpenCode agent for that child. Each Retinue MCP server session keeps up to 3 active child agents by default. A spawn beyond the session limit closes the oldest active child and returns `evictedJobId`; a spawn beyond the machine-level budget for the shared `RETINUE_STATE_DIR` returns `resource_exhausted` without creating another backend child. Deployments should tune persistent concurrency with `RETINUE_MAX_CONCURRENT_AGENTS` and `RETINUE_GLOBAL_AGENT_BUDGET` in the MCP environment because packaged config files can be overwritten by plugin refreshes. Do not pass backend, model, provider, profile, OpenCode server, `access_mode`, or `bash_policy` choices in tool arguments.
