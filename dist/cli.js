...
async function main() {
    ...
    switch (command) {
        ...
        case "run": {
            ...
            writeJson(await supervisor.run({
                cwd,
                prompt,
                name: flags.name,
                resume: flags.resume,
        ...
        case "wait": {
            const [jobId, ...rest] = args;
            const flags = parseFlags(rest);
            writeJson(await supervisor.wait(required(jobId, "jobId"), {
                timeoutMs: flags["timeout-ms"] ? Number(flags["timeout-ms"]) : undefined
            }));
            return;
        }
        ...
        case "continue": {
            const flags = parseFlags(args);
            writeJson(await supervisor.continueJob({
                cwd: required(flags.cwd, "--cwd"),
                prompt: required(flags.prompt, "--prompt"),
                jobId: flags["job-id"],
        ...
        case "peek": {
            const [jobId, ...rest] = args;
            const flags = parseFlags(rest);
            writeJson(await supervisor.peek(required(jobId, "jobId"), {
        ...
        case "cleanup": {
            const flags = parseFlags(args);
            writeJson(await supervisor.cleanup({
                olderThanMs: flags["older-than-ms"] ? Number(flags["older-than-ms"]) : undefined
            }));
            return;
        }
...
async function createSupervisorFromEnv(global) {
