...
If `retinue_wait_agent` returns `running`, treat it as a workflow event, not a dead end. The response includes `diagnostic`, `stateDir`, `tracePath`, `jobDir`, `promptPath`, `stdoutPath`, `stderrPath`, and bounded stdout/stderr tails. Inspect `diagnostic` first, then the tail fields; they usually contain recent backend state without requiring a separate filesystem read. Complex OpenCode `plan` jobs can spend several minutes in tool-call rounds before producing final text, so keep polling unless the task reaches a terminal state. If the result is `stalled`, treat it as attention-required rather than successful output; read the returned `result` and diagnostics before deciding whether to close, continue, or respawn.
