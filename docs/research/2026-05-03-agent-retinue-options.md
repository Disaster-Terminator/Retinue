...
| `mkXultra/ai-cli-mcp` | Closest existing job-retinue shape: background CLI agents, wait/result/peek/kill/cleanup style | Very small project: 12 stars, 6 forks as checked on 2026-05-03; needs security and Windows behavior review before trusting |
...
Do build or adapt a narrow local retinue if no existing option passes the security and lifecycle checks. The smallest useful version should expose `run`, `status`, `wait`, `result`, `kill`, and `cleanup`, persist job state on disk, and avoid permission bypass by default.

## Implementation Decision

This repository now implements the narrow local retinue path instead of adopting a community MCP bridge wholesale.
...
- Claude Code official docs support `claude -p`, `--output-format json`, `--output-format stream-json`, `--resume`, `--continue`, `--session-id`, `--max-turns`, and explicit permission modes. These are enough for a narrow job retinue without building a terminal harness first.
