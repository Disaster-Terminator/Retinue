...
- Backend, model, provider, profile, and OpenCode server choices remain deployment/runtime configuration. `agent` is the only per-spawn OpenCode control exposed by the product tool.
