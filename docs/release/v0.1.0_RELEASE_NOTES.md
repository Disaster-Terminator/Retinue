...
After the initial v0.1.0 baseline, Retinue added OpenCode soft-stall recovery. Recoverable no-final-text stalls are now deferred within the active wait timeout, and Retinue submits one no-tools final-answer recovery prompt to the child session. The default MCP wait ceiling is 180 seconds so the default 75-second soft-stall detectors have enough room for that rescue attempt. Hard stalls still return immediately for provider errors and read-only patch/write intent.
