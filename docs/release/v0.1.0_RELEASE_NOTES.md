...
- `retinue_wait_agent` returns the child result when the task is terminal. While the task is still running, it returns job paths and bounded stdout/stderr tails so the caller can decide whether to wait, inspect logs, or close the task. OpenCode empty-output assistant rounds are reported as `stalled` instead of being treated as successful results.
