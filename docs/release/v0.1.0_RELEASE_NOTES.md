...
- Retinue does not expose backend/profile/model/OpenCode server choices as per-call tool arguments. It exposes `agent` and `access_mode` so callers can choose an OpenCode agent for one child and, when needed, force either Retinue read-only mode or the active OpenCode profile.
