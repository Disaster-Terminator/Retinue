...
- Default active-child slots from packaged config: `3`; deployments can override the per-session limit with `RETINUE_MAX_CONCURRENT_AGENTS` and the shared machine-level budget with `RETINUE_GLOBAL_AGENT_BUDGET`.
