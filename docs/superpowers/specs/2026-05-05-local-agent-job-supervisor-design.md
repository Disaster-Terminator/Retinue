# Local Agent Job Supervisor Design

## Purpose

`supervisor` is moving from a Claude Code-specific spawn helper to a local agent job supervisor. The project should keep the lifecycle semantics already proven by the Claude Code backend while adding a backend contract that can support OpenCode without copying Claude-specific assumptions.
...
The existing `SupervisorApi` can remain the public lifecycle boundary while the implementation moves runtime-specific work behind adapters. This reduces risk: existing `claude_*` tools can stay compatible while `opencode_*` tools are added explicitly.
...
- attach to an explicit `SUPERVISOR_OPENCODE_BASE_URL` when provided
...
- expose bounded result artifacts through the same supervisor result model
- preserve OpenCode-owned provider/model configuration

OpenCode permission bypass flags must not be exposed through normal supervisor inputs.
