...
- If the user prompt already provides enough facts, the default read-only child is instructed to answer from those facts instead of opening repository tools. Repository inspection is bounded to a small number of `read`/`grep`/`glob` calls before a final text answer.
- Default read-only children cannot inspect git history, staged changes, uncommitted diffs, or the latest commit unless the caller provides the relevant file paths or content. Use `access_mode: "profile"` only when git commands are intentionally acceptable.
